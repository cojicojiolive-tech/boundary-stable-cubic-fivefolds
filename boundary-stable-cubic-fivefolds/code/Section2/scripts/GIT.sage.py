...
# This file was *autogenerated* from the file GIT.sage
from sage.all_cmdline import *   # import sage library

_sage_const_7 = Integer(7); _sage_const_3 = Integer(3); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_5_000_000 = Integer(5_000_000); _sage_const_2 = Integer(2)#!/usr/bin/env sage
...
def lcm_list(L):
    L = [int(abs(x)) for x in L if int(abs(x)) != _sage_const_0 ]
    if not L:
        return _sage_const_1 
    from functools import reduce
    return reduce(lcm2, L, _sage_const_1 )
...
def compute_normal_vectors(dim, deg, out_path="normalVectors-sage.m",
                           progress_interval=_sage_const_5_000_000 ):
    indexset = generate_index_set(dim, deg)
    ones = [_sage_const_1 ]*dim

    seen = set()
    count = _sage_const_0 

    for block in combinations(indexset, dim-_sage_const_2 ):   # Mathematica: equivalent to NextSubset
        rows = [ones] + [list(u) for u in block]  # put {1,1,...,1} as the top row
        A = matrix(QQ, rows)

        if A.rank() == dim - _sage_const_1 :
            ker = A.right_kernel()
            if ker.dimension() == _sage_const_1 :              # use only when the nullspace has dimension 1
                v = ker.basis()[_sage_const_0 ]
                seen.add(primitive_sorted(v))

        count += _sage_const_1 
        if count % progress_interval == _sage_const_0 :
...
def _canon_set_of_vectors(S: Iterable[Iterable[int]]) -> Tuple[Tuple[int, Ellipsis], Ellipsis]:
    """
...
def make_positive_with_r(r: List[int]):
    ...
    def positive(u: List[int]) -> bool:
        return sum(ui * ri for ui, ri in zip(u, r)) >= _sage_const_0 
    return positive
...
def weak_compositions(n: int, k: int):
    ...
    for i in range(n + _sage_const_1 ):
        for rest in weak_compositions(n - i, k - _sage_const_1 ):
            yield (i,) + rest
...
def main():
    ...
    if os.path.exists(IX_PATH):
        ...
        DEGREE = _sage_const_3   # e.g., cubic; for quartic set 4
        DIM = len(normalVectors[_sage_const_0 ])
...
if __name__ == "__main__":
    ...
    PROGRESS_INTERVAL = _sage_const_5_000_000    # e.g., set 100_000 for more frequent progress output
